class Solution {
public:
    int maxProductPath(vector<vector<int>>& grid) {
        int m = grid.size();
        int n = grid[0].size();
        const long long MOD = 1e9+7;

        vector<vector<long long>> maxProd(m, vector<long long>(n));
        vector<vector<long long>> minProd(m, vector<long long>(n));

        maxProd[0][0] = minProd[0][0] = grid[0][0];

        for(int i = 1;i < m;i++){
            maxProd[i][0] = minProd[i][0] = maxProd[i-1][0] * grid[i][0];
        }
        for(int j = 1;j < n;j++){
            maxProd[0][j] = minProd[0][j] = maxProd[0][j-1] * grid[0][j];
        }

        for(int i = 1;i < m;i++){
            for(int j = 1;j < n;j++){
                long val = grid[i][j];

                long maxDown = maxProd[i-1][j] * val;
                long minDown = minProd[i-1][j] * val;
                long maxRight = maxProd[i][j-1] * val;
                long minRight = minProd[i][j-1] * val;

                maxProd[i][j] = max({maxDown,minDown,maxRight,minRight});
                minProd[i][j] = min({maxDown,minDown,maxRight,minRight});
            }
        }

        long ans = maxProd[m-1][n-1];
        if(ans < 0) return -1;
        return ans % MOD;
    }
};